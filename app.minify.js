...
const state=loadState()||{meta:{title:'',date:todayISO(),thresholds:{green:0.030,yellow:0.040},appVersion:APP_VERSION},settings:{useMedian:!0},setups:[],active:0,};els.title.value=state.meta.title;els.date.value=state.meta.date;els.aggMedian.checked=!!state.settings.useMedian;els.aggMean.checked=!state.settings.useMedian;els.title.addEventListener('input',commitMeta);els.date.addEventListener('input',commitMeta);els.btnNewSetup.addEventListener('click',()=>{addSetup();render();saveState()});els.btnAddBs.addEventListener('click',()=>{addRow('BS');render();saveState()});els.btnAddFs.addEventListener('click',()=>{addRow('FS');render();saveState()});els.btnLock.addEventListener('click',()=>{toggleLock();render();saveState()});els.btnExportAll.addEventListener('click',exportAllPrintable);els.btnReset.addEventListener('click',hardReset);els.toolsBtn?.addEventListener('click',openTools);els.toolsBackdrop?.addEventListener('click',closeTools);els.toolsClose?.addEventListener('click',closeTools);els.scCompute?.addEventListener('click',computeSlopeCheck);els.scClear?.addEventListener('click',clearSlopeCheck);els.slcCompute?.addEventListener('click',computeSlopeSolve);els.slcClear?.addEventListener('click',clearSlopeSolve);els.btnAddExistingBs?.addEventListener('click',openBsPicker);els.bsPickerBackdrop?.addEventListener('click',closeBsPicker);els.bsPickerClose?.addEventListener('click',closeBsPicker);els.bsPickerCancel?.addEventListener('click',closeBsPicker);els.bsPickerList?.addEventListener('click',(e)=>{const btn=e.target.closest('button.bs-btn');if(!btn)return;insertBsFromData(btn.dataset.station,sanitizeNumber(btn.dataset.elev))});els.aggMedian.addEventListener('change',()=>{const s=activeSetup();s.useMedian=!0;recompute(s);render();saveState()});els.aggMean.addEventListener('change',()=>{const s=activeSetup();s.useMedian=!1;recompute(s);render();saveState()});if(state.setups.length===0)addSetup();render();function addSetup(){state.setups.push({locked:!1,rows:[],useMedian:!0});state.active=state.setups.length-1}
...
function render(){els.tabs.innerHTML='';state.setups.forEach((s,i)=>{const b=document.createElement('button');b.className='chip';b.textContent=`Setup ${i + 1}${s.locked ? ' 🔒' : ''}`;if(i===state.active){b.style.borderColor='#60a5fa';b.style.color='#1d4ed8'}
...
function deltaMmText(asb,design){if(!isNum(asb)||!isNum(design))return'—';const mm=Math.round((asb-design)*1000);if(mm===0)return `<span class="delta-zero">0 mm OK</span>`;const cls=mm>0?'delta-up':'delta-down';const arrow=mm>0?'▲':'▼';return `<span class="${cls}">${Math.abs(mm)} mm ${arrow}</span>`}
...
const fmtM=v=>isNum(v)?`${num3(v)} m`:'—';const fmtP=v=>isNum(v)?`${num3(v)}%`:'—';const bold=(k,txt)=>(solvedKey===k?`<b>${txt}</b>`:txt);els.slcResults.innerHTML=`
