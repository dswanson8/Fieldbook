...
function blankScSet(label){return{label:label||'',startSta:'',endSta:'',dStart:null,dEnd:null,aStart:null,aEnd:null,len:null}}
...
btn.addEventListener('click',()=>{saveScFromInputs();state.scActive=i;loadScIntoInputs();renderScTabs();saveState();toast(`Switched to ${btn.textContent}`)});let holdTimer=null;const startHold=()=>{holdTimer=setTimeout(()=>{if(confirm(`Delete ${set.label || `Setup ${i+1}`}?`)){if(state.scSets.length>1){state.scSets.splice(i,1);if(state.scActive>=state.scSets.length){state.scActive=state.scSets.length-1}
