...
function renderGradeSpots(){if(!els.gradeSpotsBody)return;if(!Array.isArray(state.gradeSpots))state.gradeSpots=[];const drop=sanitizeNumber((els.invDrop?.value??'').trim());els.gradeSpotsBody.innerHTML='';state.gradeSpots.forEach((r,i)=>{const tr=document.createElement('tr');tr.appendChild(cellEditText(r,'sta',r.sta,v=>{r.sta=v;saveGradeSpots()}));tr.appendChild(cellEditNumber(r,'dInv',r.dInv,v=>{r.dInv=v;saveGradeSpots();renderGradeSpots()},!0));const tdTop=document.createElement('td');tdTop.className='cell-edit';tdTop.style.display='flex';tdTop.style.flexDirection='column';tdTop.style.alignItems='center';const inpTop=document.createElement('input');inpTop.type='text';inpTop.className='cell-input hidden';inpTop.inputMode='decimal';inpTop.value=isNum(r.asbTop)?String(r.asbTop):(r.asbTop??'');inpTop.addEventListener('blur',()=>{const raw=inpTop.value.trim();const n=mmToMeters(raw);r.asbTop=n;saveGradeSpots();renderGradeSpots()});tdTop.appendChild(inpTop);const calcLine=document.createElement('div');calcLine.className='mini-hint';if(isNum(r.asbTop)&&isNum(drop)){const asbInv=r.asbTop-drop;calcLine.innerHTML=`${num3(r.asbTop)} − ${num3(drop)} = <b>${num3(asbInv)}</b>`}else{calcLine.textContent='Inv —'}
tdTop.appendChild(calcLine);const pickBtn=document.createElement('button');pickBtn.type='button';pickBtn.textContent='Pick';pickBtn.style.cssText='margin-top:6px;padding:6px 10px;width:auto;';pickBtn.addEventListener('click',()=>openAsbPicker(i));tdTop.appendChild(pickBtn);tdTop.appendChild(inpTop);tdTop.appendChild(pickBtn);tr.appendChild(tdTop);const tdDelta=document.createElement('td');tdDelta.className='cell-calc';let html='—';if(isNum(r.dInv)&&isNum(r.asbTop)&&isNum(drop)){const asbInv=r.asbTop-drop;html=deltaMmText(asbInv,r.dInv)}
