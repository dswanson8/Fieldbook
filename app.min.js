...
function addGradeRow(){state.gradeSpots.push({sta:'',dInv:null,asbTop:null,pipeType:'storm',shotMode:'top'})}
...
tdDelta.innerHTML=html;tr.appendChild(tdDelta);const tdGps=document.createElement('td');tdGps.className='cell-calc';tdGps.style.display='flex';tdGps.style.flexDirection='column';tdGps.style.alignItems='center';tdGps.style.padding='4px';const gpsElevLine=document.createElement('div');gpsElevLine.className='mini-hint';if(isNum(r.gpsElev)){gpsElevLine.innerHTML=`GPS ${num3(r.gpsElev)}`}else{gpsElevLine.textContent='GPS —'}
tdGps.appendChild(gpsElevLine);const gpsCalcLine=document.createElement('div');gpsCalcLine.className='mini-hint';let gpsInv=null;if(isNum(r.gpsElev)&&isNum(gpsDrop)){gpsInv=r.gpsElev-gpsDrop;gpsCalcLine.innerHTML=`− ${num3(gpsDrop)} = <b>${num3(gpsInv)}</b>`}else{gpsCalcLine.textContent='invert —'}
tdGps.appendChild(gpsCalcLine);const gpsDeltaLine=document.createElement('div');gpsDeltaLine.className='mini-hint';if(isNum(r.dInv)&&isNum(gpsInv)){gpsDeltaLine.innerHTML=deltaMmText(gpsInv,r.dInv)}else{gpsDeltaLine.textContent='Δ —'}
