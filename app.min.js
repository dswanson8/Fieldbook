(function(){const APP_VERSION='2025-11-25';const STORAGE_KEY='dfb_v3_state';const GRADE_SPOTS_KEY='dfb_grade_spots_v1';const STORM_DROP_KEY='dfb_storm_drop_v1';const SAN_DROP_KEY='dfb_san_drop_v1';const STORM_GPS_DROP_KEY='dfb_storm_gps_drop_v1';const SAN_GPS_DROP_KEY='dfb_san_gps_drop_v1';const INV_DROP_KEY='dfb_inv_drop_v1';const GPS_DROP_KEY='dfb_gps_drop_v1';const els={title:byId('jobTitle'),date:byId('jobDate'),tabs:byId('setupTabs'),gridBody:byId('gridBody'),btnNewSetup:byId('newSetupBtn'),btnAddBs:byId('addBsBtn'),btnAddFs:byId('addFsBtn'),btnLock:byId('lockToggle'),btnExportAll:byId('exportAllBtn'),btnReset:byId('resetBtn'),aggMedian:byId('aggMedian'),aggMean:byId('aggMean'),summary:byId('summary'),toolsBtn:byId('toolsBtn'),toolsSheet:byId('toolsSheet'),toolsBackdrop:byId('toolsBackdrop'),toolsClose:byId('toolsClose'),scStartSta:byId('scStartSta'),scEndSta:byId('scEndSta'),scDStart:byId('scDStart'),scDEnd:byId('scDEnd'),scAStart:byId('scAStart'),scAEnd:byId('scAEnd'),scLen:byId('scLen'),scResults:byId('scResults'),scCompute:byId('scCompute'),scClear:byId('scClear'),scSetsBar:byId('scSetsBar'),scDStartPick:byId('scDStartPick'),scDEndPick:byId('scDEndPick'),scAStartPick:byId('scAStartPick'),scAEndPick:byId('scAEndPick'),scPickerSheet:byId('scPickerSheet'),scPickerBackdrop:byId('scPickerBackdrop'),scPickerClose:byId('scPickerClose'),scPickerTitle:byId('scPickerTitle'),scPickerList:byId('scPickerList'),gradeSpotsBtn:byId('gradeSpotsBtn'),gradeSpotsSheet:byId('gradeSpotsSheet'),gradeSpotsBackdrop:byId('gradeSpotsBackdrop'),gradeSpotsClose:byId('gradeSpotsClose'),stormDrop:byId('stormDrop'),sanDrop:byId('sanDrop'),stormGpsDrop:byId('stormGpsDrop'),sanGpsDrop:byId('sanGpsDrop'),gradeSpotsBody:byId('gradeSpotsBody'),addGradeRowBtn:byId('addGradeRowBtn'),clearGradeRowsBtn:byId('clearGradeRowsBtn'),asbPickerSheet:byId('asbPickerSheet'),asbPickerList:byId('asbPickerList'),asbPickerClose:byId('asbPickerClose'),slcStart:byId('slcStart'),slcEnd:byId('slcEnd'),slcSlope:byId('slcSlope'),slcLen:byId('slcLen'),slcResults:byId('slcResults'),slcCompute:byId('slcCompute'),slcClear:byId('slcClear'),existingBsBar:byId('existingBsBar'),btnAddExistingBs:byId('addExistingBsBtn'),bsPickerSheet:byId('bsPickerSheet'),bsPickerBackdrop:byId('bsPickerBackdrop'),bsPickerClose:byId('bsPickerClose'),bsPickerList:byId('bsPickerList'),bsPickerUse:byId('bsPickerUse'),bsPickerCancel:byId('bsPickerCancel'),};const _toastEl=document.createElement('div');_toastEl.style.cssText=`
        position: fixed; bottom: 16px; left: 50%; transform: translateX(-50%);
        background: #1f2937; color: #ffffff; padding: 10px 16px;
        border-radius: 8px; font-size: 14px; z-index: 9999; opacity: 0;
        text-align: center; pointer-events: none; transition: opacity 0.2s, transform 0.2s;
    `;document.body.appendChild(_toastEl);let _toastTimer=null;function toast(msg){_toastEl.textContent=msg;_toastEl.style.opacity=1;_toastEl.style.transform='translateX(-50%) translateY(-4px)';if(_toastTimer)clearTimeout(_toastTimer);_toastTimer=setTimeout(()=>{_toastEl.style.opacity=0;_toastEl.style.transform='translateX(-50%) translateY(0)'},1400)}
const state=loadState()||{meta:{title:'',date:todayISO(),thresholds:{green:0.030,yellow:0.040},appVersion:APP_VERSION},settings:{useMedian:!0},setups:[],active:0,};if(!Array.isArray(state.gradeSpots))state.gradeSpots=[];els.title.value=state.meta.title;els.date.value=state.meta.date;els.aggMedian.checked=!!state.settings.useMedian;els.aggMean.checked=!state.settings.useMedian;els.title.addEventListener('input',commitMeta);els.date.addEventListener('input',commitMeta);els.btnNewSetup.addEventListener('click',()=>{addSetup();render();saveState()});els.btnAddBs.addEventListener('click',()=>{addRow('BS');render();saveState()});els.btnAddFs.addEventListener('click',()=>{addRow('FS');render();saveState()});els.btnLock.addEventListener('click',()=>{toggleLock();render();saveState()});els.btnExportAll.addEventListener('click',exportAllPrintable);els.btnReset.addEventListener('click',hardReset);els.toolsBtn?.addEventListener('click',openTools);els.toolsBackdrop?.addEventListener('click',closeTools);els.toolsClose?.addEventListener('click',closeTools);els.slcCompute?.addEventListener('click',computeSlopeSolve);els.slcClear?.addEventListener('click',clearSlopeSolve);els.addGradeRowBtn?.addEventListener('click',()=>{addGradeRow();renderGradeSpots();saveGradeSpots()});els.clearGradeRowsBtn?.addEventListener('click',()=>{if(confirm('Clear all grade spot rows?')){state.gradeSpots=[];renderGradeSpots();saveGradeSpots();toast('Cleared')}});els.stormDrop?.addEventListener('blur',()=>{const raw=(els.stormDrop.value??'').trim();const v=sanitizeNumber(raw);localStorage.setItem(STORM_DROP_KEY,JSON.stringify(isNum(v)?v:null));renderGradeSpots()});els.sanDrop?.addEventListener('blur',()=>{const raw=(els.sanDrop.value??'').trim();const v=sanitizeNumber(raw);localStorage.setItem(SAN_DROP_KEY,JSON.stringify(isNum(v)?v:null));renderGradeSpots()});els.stormGpsDrop?.addEventListener('blur',()=>{const raw=(els.stormGpsDrop.value??'').trim();const v=sanitizeNumber(raw);localStorage.setItem(STORM_GPS_DROP_KEY,JSON.stringify(isNum(v)?v:null));renderGradeSpots()});els.sanGpsDrop?.addEventListener('blur',()=>{const raw=(els.sanGpsDrop.value??'').trim();const v=sanitizeNumber(raw);localStorage.setItem(SAN_GPS_DROP_KEY,JSON.stringify(isNum(v)?v:null));renderGradeSpots()});els.gpsDrop?.addEventListener('blur',()=>{const v=sanitizeNumber((els.gpsDrop.value??'').trim());localStorage.setItem(GPS_DROP_KEY,JSON.stringify(isNum(v)?v:null));renderGradeSpots()});els.gradeSpotsBtn?.addEventListener('click',openGradeSpots);els.gradeSpotsBackdrop?.addEventListener('click',closeGradeSpots);els.gradeSpotsClose?.addEventListener('click',closeGradeSpots);els.asbPickerBackdrop?.addEventListener('click',closeAsbPicker);els.asbPickerClose?.addEventListener('click',closeAsbPicker);els.asbPickerList?.addEventListener('click',(e)=>{const btn=e.target.closest('button.bs-btn');if(!btn)return;const idx=Number(btn.dataset.targetIndex);if(!Number.isInteger(idx)||idx<0)return;const elev=sanitizeNumber(btn.dataset.elev);if(!isNum(elev))return toast('Invalid elevation');const gpsVal=sanitizeNumber(btn.dataset.gps);const row=state.gradeSpots[idx];row.asbTop=elev;row.gpsElev=isNum(gpsVal)?gpsVal:null;renderGradeSpots();saveGradeSpots();closeAsbPicker();toast('ASB elevation selected')});els.scDStartPick?.addEventListener('click',()=>openScPicker('design','scDStart'));els.scDEndPick?.addEventListener('click',()=>openScPicker('design','scDEnd'));els.scAStartPick?.addEventListener('click',()=>openScPicker('asb','scAStart'));els.scAEndPick?.addEventListener('click',()=>openScPicker('asb','scAEnd'));els.scPickerBackdrop?.addEventListener('click',closeScPicker);els.scPickerClose?.addEventListener('click',closeScPicker);els.scPickerList?.addEventListener('click',(e)=>{const btn=e.target.closest('button.bs-btn');if(!btn||!_scPickerTarget)return;const val=sanitizeNumber(btn.dataset.value);if(!isNum(val)){toast('Invalid elevation');return}
const input=els[_scPickerTarget.inputId];if(input){input.value=num3(val)}
const sta=(btn.dataset.sta||'').trim();if(sta){if(_scPickerTarget.inputId==='scDStart'||_scPickerTarget.inputId==='scAStart'){if(!els.scStartSta.value.trim()){els.scStartSta.value=sta}}else if(_scPickerTarget.inputId==='scDEnd'||_scPickerTarget.inputId==='scAEnd'){if(!els.scEndSta.value.trim()){els.scEndSta.value=sta}}}
closeScPicker()});els.btnAddExistingBs?.addEventListener('click',openBsPicker);els.bsPickerBackdrop?.addEventListener('click',closeBsPicker);els.bsPickerClose?.addEventListener('click',closeBsPicker);els.bsPickerCancel?.addEventListener('click',closeBsPicker);els.bsPickerList?.addEventListener('click',(e)=>{const btn=e.target.closest('button.bs-btn');if(!btn)return;insertBsFromData(btn.dataset.station,sanitizeNumber(btn.dataset.elev))});els.aggMedian.addEventListener('change',()=>{const s=activeSetup();s.useMedian=!0;recompute(s);render();saveState()});els.aggMean.addEventListener('change',()=>{const s=activeSetup();s.useMedian=!1;recompute(s);render();saveState()});[els.scStartSta,els.scEndSta,els.scDStart,els.scDEnd,els.scAStart,els.scAEnd,els.scLen].filter(Boolean).forEach(inp=>{inp.addEventListener('blur',()=>{saveScFromInputs();saveState()})});els.scCompute?.addEventListener('click',()=>{saveScFromInputs();const s=activeScSet();if(!s)return;s.result=computeScFor(s);renderScResultsFrom(s.result);saveState();toast('Computed')});els.scClear?.addEventListener('click',()=>{const label=activeScSet()?.label||`Setup ${state.scActive + 1}`;state.scSets[state.scActive]=blankScSet(label);loadScIntoInputs();if(els.scResults)els.scResults.innerHTML='';saveState();toast('Cleared this setup')});if(state.setups.length===0)addSetup();render();function addSetup(){state.setups.push({locked:!1,rows:[],useMedian:!0});state.active=state.setups.length-1}
...
b.addEventListener('click',()=>{state.active=i;render();saveState()});els.tabs.appendChild(b)});if(els.existingBsBar){els.existingBsBar.classList.toggle('hidden',state.active===0)}
els.btnLock.textContent=activeSetup().locked?'🔓 Unlock':'🔒 Lock';els.gridBody.innerHTML='';const s=activeSetup();els.aggMedian.checked=!!s.useMedian;els.aggMean.checked=!s.useMedian;const fragment=document.createDocumentFragment();s.rows.forEach((r,idx)=>{const tr=document.createElement('tr');const tdSta=makeCell(r,'station',r.station,s.locked);attachRowDelete(tdSta,s,idx);tr.appendChild(tdSta);tr.appendChild(r.type==='BS'?makeCell(r,'bs',r.bs,s.locked):tdDim());tr.appendChild(makeCalc(r.hi));tr.appendChild(r.type==='FS'?makeCell(r,'fs',r.fs,s.locked):tdDim());tr.appendChild(r.type==='BS'?makeCell(r,'knownElev',r.knownElev,s.locked):makeCalc(r.elev));if(r.type==='FS'){const tdGps=makeCell(r,'gps',r.gps,s.locked);if(isNum(r.delta)){const thr=state.meta.thresholds;tdGps.classList.add(r.delta<=thr.green?'qa-green':r.delta<=thr.yellow?'qa-yellow':'qa-red')}
tr.appendChild(tdGps)}else{tr.appendChild(tdDim())}
fragment.appendChild(tr)});els.gridBody.appendChild(fragment);updateSummary()}
...
tds.push(`<td${cellStyle}>${gpsHtml}</td>`);return `<tr>${tds.join('')}</tr>`}).join('');return `
        ...
        <title>${title} — All Setups</title>
        <style>
            body{font-family:Inter,system-ui,-apple-system,sans-serif;color:#111;margin:24px;}
            h1,h2{margin:0 0 6px;} h2{font-size:18px;}
            .meta{margin:0 0 12px;color:#444} .muted{color:#555}
        ...
        <body>
            <h1>${title} — All Setups</h1>
            ${sections}
            <button onclick="window.print()">Print / Save as PDF</button>
        </body></html>`;const win=window.open('','_blank');if(win){win.document.open();win.document.write(html);win.document.close()}else{toast('Pop-up blocked. Allow pop-ups to export.')}}
...
function hardReset(){if(!confirm('Clear everything?'))return;localStorage.removeItem(STORAGE_KEY);localStorage.removeItem(GRADE_SPOTS_KEY);localStorage.removeItem(INV_DROP_KEY);localStorage.removeItem(GPS_DROP_KEY);state=defaultState();render();saveState();updateMetaUI();toast('Cleared')}
...
function deltaMmText(asb,design){if(!isNum(asb)||!isNum(design))return'—';const mm=Math.round((asb-design)*1000);if(mm===0)return `<span class="delta-badge neutral">0 mm OK</span>`;const isHigh=mm>0;const arrow=isHigh?'▲':'▼';const tone=isHigh?'green':'red';return `<span class="delta-badge ${tone}">${arrow} ${Math.abs(mm)} mm ${isHigh ? 'HIGH' : 'LOW'}</span>`}
...
els.bsPickerList.className='bs-grid';els.bsPickerList.innerHTML=options.map(opt=>`
            ...
            <span class="bs-meta">Elev <span class="text-blue bold">${num3(opt.elev)}</span> • Setup ${opt.setup + 1} • ${opt.source}</span>
            </button>
...
function loadGradeSpots(){try{const raw=localStorage.getItem(GRADE_SPOTS_KEY);if(raw){const rows=JSON.parse(raw);if(Array.isArray(rows)){state.gradeSpots=rows.map(r=>({sta:r.sta||'',dInv:isNum(r.dInv)?r.dInv:null,asbTop:isNum(r.asbTop)?r.asbTop:(r.asbTop??null),gpsElev:isNum(r.gpsElev)?r.gpsElev:(r.gpsElev??null),pipeType:(r.pipeType==='san')?'san':'storm',shotMode:(r.shotMode==='inv')?'inv':'top',gpsShotMode:(r.gpsShotMode==='inv')?'inv':'top'}))}}}catch(e){}
try{const sRaw=localStorage.getItem(STORM_DROP_KEY);const legacy=localStorage.getItem(INV_DROP_KEY);let storm=null;if(sRaw){const v=JSON.parse(sRaw);if(isNum(v))storm=v}else if(legacy){const v=JSON.parse(legacy);if(isNum(v))storm=v}
if(isNum(storm)&&els.stormDrop){els.stormDrop.value=String(storm)}}catch(e){}
try{const raw=localStorage.getItem(SAN_DROP_KEY);if(raw){const v=JSON.parse(raw);if(isNum(v)&&els.sanDrop){els.sanDrop.value=String(v)}}}catch(e){}
try{const raw=localStorage.getItem(STORM_GPS_DROP_KEY);if(raw){const v=JSON.parse(raw);if(isNum(v)&&els.stormGpsDrop){els.stormGpsDrop.value=String(v)}}}catch(e){}
try{const raw=localStorage.getItem(SAN_GPS_DROP_KEY);if(raw){const v=JSON.parse(raw);if(isNum(v)&&els.sanGpsDrop){els.sanGpsDrop.value=String(v)}}}catch(e){}}
function saveGradeSpots(){try{localStorage.setItem(GRADE_SPOTS_KEY,JSON.stringify(state.gradeSpots))}catch(e){}}
function addGradeRow(){state.gradeSpots.push({sta:'',dInv:null,asbTop:null,gpsElev:null,pipeType:'storm',shotMode:'top',gpsShotMode:'top'})}
function getRodDropForRow(r){const storm=sanitizeNumber((els.stormDrop?.value??'').trim());const san=sanitizeNumber((els.sanDrop?.value??'').trim());if(r.pipeType==='san'){return isNum(san)?san:null}
return isNum(storm)?storm:null}
function getGpsDropForRow(r){const storm=sanitizeNumber((els.stormGpsDrop?.value??'').trim());const san=sanitizeNumber((els.sanGpsDrop?.value??'').trim());if(r.pipeType==='san'){return isNum(san)?san:null}
return isNum(storm)?storm:null}
function computeAsbInvForRow(r){if(!r||!isNum(r.asbTop))return null;if(r.shotMode==='inv'){return r.asbTop}
const rowDrop=getRodDropForRow(r);if(!isNum(rowDrop))return null;return r.asbTop-rowDrop}
function renderGradeSpots(){if(!els.gradeSpotsBody)return;if(!Array.isArray(state.gradeSpots))state.gradeSpots=[];const fragment=document.createDocumentFragment();const gpsDrop=sanitizeNumber((els.gpsDrop?.value??'').trim());els.gradeSpotsBody.innerHTML='';state.gradeSpots.forEach((r,i)=>{if(!r.pipeType)r.pipeType='storm';if(!r.shotMode)r.shotMode='top';if(!r.gpsShotMode)r.gpsShotMode='top';const metaTr=document.createElement('tr');metaTr.className='grade-row-meta';const metaTd=document.createElement('td');metaTd.colSpan=4;metaTr.appendChild(metaTd);const bar=document.createElement('div');bar.className='grade-meta-bar';metaTd.appendChild(bar);const createChip=(label,active,onClick)=>{const s=document.createElement('span');s.className='grade-chip'+(active?' grade-chip--active':'');s.textContent=label;s.addEventListener('click',onClick);return s};const groupPipe=document.createElement('div');groupPipe.className='grade-toggle-group';groupPipe.innerHTML='<span class="grade-toggle-label">Pipe</span>';bar.appendChild(groupPipe);groupPipe.appendChild(createChip('STM',r.pipeType==='storm',()=>{r.pipeType='storm';saveGradeSpots();renderGradeSpots()}));groupPipe.appendChild(createChip('SAN',r.pipeType==='san',()=>{r.pipeType='san';saveGradeSpots();renderGradeSpots()}));const groupMode=document.createElement('div');groupMode.className='grade-toggle-group';groupMode.innerHTML='<span class="grade-toggle-label">Shot</span>';bar.appendChild(groupMode);groupMode.appendChild(createChip('TOP',r.shotMode==='top',()=>{r.shotMode='top';saveGradeSpots();renderGradeSpots()}));groupMode.appendChild(createChip('INV',r.shotMode==='inv',()=>{r.shotMode='inv';saveGradeSpots();renderGradeSpots()}));fragment.appendChild(metaTr);const tr=document.createElement('tr');const tdSta=cellEditText(r,'sta',r.sta,v=>{r.sta=v;saveGradeSpots()});attachGradeRowDelete(tdSta,i);tr.appendChild(tdSta);tr.appendChild(cellEditNumber(r,'dInv',r.dInv,v=>{r.dInv=v;saveGradeSpots();renderGradeSpots()},!0));const tdTop=document.createElement('td');tdTop.className='cell-edit';tdTop.style.cssText='display:flex; flex-direction:column; align-items:center; padding:4px;';const inpTop=document.createElement('input');inpTop.className='cell-input cell-input--manual';inpTop.inputMode='decimal';inpTop.value=isNum(r.asbTop)?String(r.asbTop):(r.asbTop??'');inpTop.addEventListener('blur',()=>{r.asbTop=mmToMeters(inpTop.value.trim());saveGradeSpots();renderGradeSpots()});tdTop.appendChild(inpTop);const calcLine=document.createElement('div');calcLine.className='mini-hint';let asbInv=null;const rowDrop=getRodDropForRow(r);if(isNum(r.asbTop)){if(r.shotMode==='inv'){asbInv=r.asbTop;calcLine.innerHTML=`ASB invert = <b>${num3(asbInv)}</b>`}else if(r.shotMode==='top'&&isNum(rowDrop)){asbInv=r.asbTop-rowDrop;calcLine.innerHTML=`− ${num3(rowDrop)} = <b>${num3(asbInv)}</b>`}else{calcLine.textContent='invert —'}}else{calcLine.textContent='invert —'}
tdTop.appendChild(calcLine);const pickBtn=document.createElement('button');pickBtn.textContent='Pick';pickBtn.style.cssText='margin: 6px auto 2px; padding:4px 18px; width:auto; font-size: 8px;';pickBtn.type='button';pickBtn.addEventListener('click',()=>openAsbPicker(i));tdTop.appendChild(pickBtn);tr.appendChild(tdTop);const tdDelta=document.createElement('td');tdDelta.className='cell-calc';tdDelta.innerHTML=(isNum(r.dInv)&&isNum(asbInv))?deltaMmText(asbInv,r.dInv):'—';tr.appendChild(tdDelta);const tdGps=document.createElement('td');tdGps.className='cell-calc';const gpsInput=document.createElement('input');gpsInput.className='cell-edit cell-input--manual';gpsInput.placeholder='GPS Elev';gpsInput.inputMode='decimal';gpsInput.value=isNum(r.gpsElev)?num3(r.gpsElev):'';gpsInput.addEventListener('blur',()=>{const v=mmToMeters((gpsInput.value??'').trim());r.gpsElev=isNum(v)?v:null;saveGradeSpots();renderGradeSpots()});tdGps.appendChild(gpsInput);let gpsInv=null;const gpsFormula=document.createElement('div');gpsFormula.className='mini-hint';if(isNum(r.gpsElev)){if(r.gpsShotMode==='inv'){gpsInv=r.gpsElev}else{const gDrop=getGpsDropForRow(r);if(isNum(gDrop)){gpsInv=r.gpsElev-gDrop;gpsFormula.innerHTML=`− ${num3(gDrop)} = <b>${num3(gpsInv)}</b>`}else{gpsFormula.textContent=`${num3(r.gpsElev)} − (no GPS drop)`}}}
if(gpsFormula.innerHTML)tdGps.appendChild(gpsFormula);const gpsDeltaLine=document.createElement('div');gpsDeltaLine.className='mini-hint';gpsDeltaLine.innerHTML=(isNum(r.dInv)&&isNum(gpsInv))?deltaMmText(gpsInv,r.dInv):'Δ —';tdGps.appendChild(gpsDeltaLine);const gpsModeChip=document.createElement('span');gpsModeChip.className='grade-chip grade-chip--active';gpsModeChip.textContent=(r.gpsShotMode==='inv')?'GPS INV':'GPS TOP';gpsModeChip.style.marginTop='2px';gpsModeChip.addEventListener('click',()=>{r.gpsShotMode=(r.gpsShotMode==='inv')?'top':'inv';saveGradeSpots();renderGradeSpots()});tdGps.appendChild(gpsModeChip);tr.appendChild(tdGps);fragment.appendChild(tr)});els.gradeSpotsBody.appendChild(fragment)}
...
let _gradeSpotTargetIndex=-1;let _scPickerTarget=null;function openAsbPicker(targetIndex){_gradeSpotTargetIndex=targetIndex;const options=gatherAsbElevStations();if(!options.length){return toast('No stations with elevation found')}
els.asbPickerList.className='bs-grid';els.asbPickerList.innerHTML=options.map(opt=>{const elevText=isNum(opt.elev)?num3(opt.elev):'—';const gpsText=isNum(opt.gps)?` • GPS ${num3(opt.gps)}`:'';return `
            <button class="bs-btn"
            ...
            <div>${escapeHtml(opt.station || '(no STA)')}</div>
            <span class="bs-meta">Elev <span class="text-blue bold">${elevText}</span> • Setup ${opt.setup + 1} • ${opt.source}<span class="text-orange bold">${gpsText}</span></span>
            </button>
...
function openScPicker(mode,inputId){_scPickerTarget={mode,inputId};if(!Array.isArray(state.gradeSpots)||!state.gradeSpots.length){toast('No grade sheet rows to pick from');_scPickerTarget=null;return}
const options=[];if(mode==='design'){state.gradeSpots.forEach((r,idx)=>{if(isNum(r.dInv)){options.push({rowIndex:idx,sta:r.sta||'',value:r.dInv})}})}else if(mode==='asb'){state.gradeSpots.forEach((r,idx)=>{const inv=computeAsbInvForRow(r);if(isNum(inv)){options.push({rowIndex:idx,sta:r.sta||'',value:inv})}})}
if(!options.length){toast(mode==='design'?'No design elevations found':'No ASB inverts found');_scPickerTarget=null;return}
els.scPickerList.className='bs-grid';els.scPickerList.innerHTML=options.map(opt=>{const sta=opt.sta||'(no STA)';const label=mode==='design'?`Design Inv <span class="text-blue bold">${num3(opt.value)}</span>`:`ASB Inv <span class="text-orange bold">${num3(opt.value)}</span>`;return `
            <button class="bs-btn"
                    data-value="${opt.value}"
                    data-sta="${escapeHtml(opt.sta || '')}">
            <div>${escapeHtml(sta)}</div>
            <span class="bs-meta">${label}</span>
            </button>`}).join('');if(els.scPickerTitle){els.scPickerTitle.textContent=mode==='design'?'Pick Design Elevation':'Pick ASB Elevation'}
els.scPickerSheet.classList.remove('hidden');els.scPickerSheet.setAttribute('aria-hidden','false')}
function closeScPicker(){els.scPickerSheet?.classList.add('hidden');els.scPickerSheet?.setAttribute('aria-hidden','true');_scPickerTarget=null}
...
return out}
function renderScResultsFrom(out){const pct=v=>isNum(v)?`${num3(v)}%`:'—';const s=activeScSet&&activeScSet();const startBadge=(s&&isNum(s.aStart)&&isNum(s.dStart))?deltaMmText(s.aStart,s.dStart):'—';const endBadge=(s&&isNum(s.aEnd)&&isNum(s.dEnd))?deltaMmText(s.aEnd,s.dEnd):'—';if(els.scResults){els.scResults.innerHTML=`<div><b>Design Slope:</b> ${pct(out.designSlope)} ${out.designRatio ? `(${out.designRatio})` : ''}</div>
                <div><b>ASB Slope:</b> ${pct(out.asbSlope)} ${out.asbRatio ? `(${out.asbRatio})` : ''}</div>
                <div><b>Δ Slope:</b> ${pct(out.deltaSlope)}</div>
                <div><b>Δ Start Elev:</b> ${startBadge}</div>
                <div><b>Δ End Elev:</b> ${endBadge}</div>`}}
