...
function addGradeRow(){state.gradeSpots.push({sta:'',dInv:null,asbTop:null,pipeType:'storm',shotMode:'top',gpsShotMode:'top'})}
...
tdTop.appendChild(calcLine);const pickBtn=document.createElement('button');pickBtn.type='button';pickBtn.textContent='Pick';pickBtn.style.cssText='margin: 2px 0px 2px; padding:4px 18px;width:auto;font-size: 8px;';pickBtn.addEventListener('click',()=>openAsbPicker(i));tdTop.appendChild(pickBtn);tr.appendChild(tdTop);const tdDelta=document.createElement('td');tdDelta.className='cell-calc';let html='—';if(isNum(r.dInv)&&isNum(asbInv)){html=deltaMmText(asbInv,r.dInv)}
tdDelta.innerHTML=html;tr.appendChild(tdDelta);const tdGps=document.createElement('td');tdGps.className='cell-calc';let gpsInv=null;const gpsLine=document.createElement('div');gpsLine.className='mini-hint';if(isNum(r.gpsElev)){if(r.gpsShotMode==='inv'){gpsInv=r.gpsElev;gpsLine.innerHTML=`GPS invert = <b>${num3(gpsInv)}</b>`}else{const gpsDrop=getGpsDropForRow(r);if(isNum(gpsDrop)){gpsInv=r.gpsElev-gpsDrop;gpsLine.innerHTML=`GPS ${num3(r.gpsElev)} − ${num3(gpsDrop)} = <b>${num3(gpsInv)}</b>`}else{gpsLine.textContent=`GPS ${num3(r.gpsElev)} → invert —`}}}else{gpsLine.textContent='GPS —'}
