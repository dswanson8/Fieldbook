...
const options=[];if(mode==='design'){state.gradeSpots.forEach((r,idx)=>{if(isNum(r.dInv)){options.push({rowIndex:idx,sta:r.sta||'',value:r.dInv})}})}else if(mode==='asb'){const drop=sanitizeNumber((els.invDrop?.value??'').trim());if(!isNum(drop)){toast('Enter Top→Invert subtract first');_scPickerTarget=null;return}
state.gradeSpots.forEach((r,idx)=>{if(isNum(r.asbTop)){const inv=r.asbTop-drop;options.push({rowIndex:idx,sta:r.sta||'',value:inv})}})}
