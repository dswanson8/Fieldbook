...
saveGradeSpots();renderGradeSpots()});tdGps.appendChild(gpsInput);let gpsInv=null;const gpsFormula=document.createElement('div');gpsFormula.className='mini-hint';if(isNum(r.gpsElev)){if(r.gpsShotMode==='inv'){gpsInv=r.gpsElev}else{const gpsDrop=getGpsDropForRow(r);if(isNum(gpsDrop)){gpsInv=r.gpsElev-gpsDrop;gpsFormula.innerHTML=`− ${num3(gpsDrop)} = <b>${num3(gpsInv)}</b>`}else{gpsFormula.textContent=`${num3(r.gpsElev)} − (no GPS drop)`}}}
if(gpsFormula.textContent){tdGps.appendChild(gpsFormula)}
const gpsDeltaLine=document.createElement('div');gpsDeltaLine.className='mini-hint';if(isNum(r.dInv)&&isNum(gpsInv)){gpsDeltaLine.innerHTML=deltaMmText(gpsInv,r.dInv)}else{gpsDeltaLine.textContent='Δ —'}
