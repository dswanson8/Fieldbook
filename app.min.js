...
function renderGradeSpots(){if(!els.gradeSpotsBody)return;if(!Array.isArray(state.gradeSpots))state.gradeSpots=[];const gpsDrop=sanitizeNumber((els.gpsDrop?.value??'').trim());els.gradeSpotsBody.innerHTML='';state.gradeSpots.forEach((r,i)=>{if(!r.pipeType)r.pipeType='storm';if(!r.shotMode)r.shotMode='top';if(!r.gpsShotMode)r.gpsShotMode='top';const metaTr=document.createElement('tr');metaTr.className='grade-row-meta';const metaTd=document.createElement('td');metaTd.colSpan=4;metaTr.appendChild(metaTd);const bar=document.createElement('div');bar.className='grade-meta-bar';metaTd.appendChild(bar);const groupPipe=document.createElement('div');groupPipe.className='grade-toggle-group';bar.appendChild(groupPipe);const pipeLabel=document.createElement('span');pipeLabel.className='grade-toggle-label';pipeLabel.textContent='Pipe';groupPipe.appendChild(pipeLabel);const chipStorm=document.createElement('span');chipStorm.className='grade-chip'+(r.pipeType==='storm'?' grade-chip--active':'');chipStorm.textContent='STM';chipStorm.addEventListener('click',()=>{r.pipeType='storm';saveGradeSpots();renderGradeSpots()});groupPipe.appendChild(chipStorm);const chipSan=document.createElement('span');chipSan.className='grade-chip'+(r.pipeType==='san'?' grade-chip--active':'');chipSan.textContent='SAN';chipSan.addEventListener('click',()=>{r.pipeType='san';saveGradeSpots();renderGradeSpots()});groupPipe.appendChild(chipSan);const groupMode=document.createElement('div');groupMode.className='grade-toggle-group';bar.appendChild(groupMode);const modeLabel=document.createElement('span');modeLabel.className='grade-toggle-label';modeLabel.textContent='Shot';groupMode.appendChild(modeLabel);const chipTop=document.createElement('span');chipTop.className='grade-chip'+(r.shotMode==='top'?' grade-chip--active':'');chipTop.textContent='TOP';chipTop.addEventListener('click',()=>{r.shotMode='top';saveGradeSpots();renderGradeSpots()});groupMode.appendChild(chipTop);const chipInv=document.createElement('span');chipInv.className='grade-chip'+(r.shotMode==='inv'?' grade-chip--active':'');chipInv.textContent='INV';chipInv.addEventListener('click',()=>{r.shotMode='inv';saveGradeSpots();renderGradeSpots()});groupMode.appendChild(chipInv);els.gradeSpotsBody.appendChild(metaTr);const tr=document.createElement('tr');const tdSta=cellEditText(r,'sta',r.sta,v=>{r.sta=v;saveGradeSpots()});attachGradeRowDelete(tdSta,i);tr.appendChild(tdSta);tr.appendChild(cellEditNumber(r,'dInv',r.dInv,v=>{r.dInv=v;saveGradeSpots();renderGradeSpots()},!0));const tdTop=document.createElement('td');tdTop.className='cell-edit';tdTop.style.display='';tdTop.style.flexDirection='column';tdTop.style.alignItems='center';tdTop.style.padding='4px';const inpTop=document.createElement('input');inpTop.type='text';inpTop.className='cell-input cell-input--manual';inpTop.inputMode='decimal';inpTop.value=isNum(r.asbTop)?String(r.asbTop):(r.asbTop??'');inpTop.addEventListener('blur',()=>{const raw=inpTop.value.trim();const n=mmToMeters(raw);r.asbTop=n;saveGradeSpots();renderGradeSpots()});tdTop.appendChild(inpTop);const calcLine=document.createElement('div');calcLine.className='mini-hint';let asbInv=null;const rowDrop=getRodDropForRow(r);if(isNum(r.asbTop)){if(r.shotMode==='inv'){asbInv=r.asbTop;calcLine.innerHTML=`ASB invert = <b>${num3(asbInv)}</b>`}else if(r.shotMode==='top'&&isNum(rowDrop)){asbInv=r.asbTop-rowDrop;calcLine.innerHTML=`− ${num3(rowDrop)} = <b>${num3(asbInv)}</b>`}else{calcLine.textContent='invert —'}}else{calcLine.textContent='invert —'}
tdTop.appendChild(calcLine);const pickBtn=document.createElement('button');pickBtn.type='button';pickBtn.textContent='Pick';pickBtn.style.cssText='margin: 6px auto 2px; padding:4px 18px;width:auto;font-size: 8px;';pickBtn.addEventListener('click',()=>openAsbPicker(i));tdTop.appendChild(pickBtn);tr.appendChild(tdTop);const tdDelta=document.createElement('td');tdDelta.className='cell-calc';let html='—';if(isNum(r.dInv)&&isNum(asbInv)){html=deltaMmText(asbInv,r.dInv)}
tdDelta.innerHTML=html;tr.appendChild(tdDelta);const tdGps=document.createElement('td');tdGps.className='cell-calc';const gpsInput=document.createElement('input');gpsInput.type='text';gpsInput.className='cell-edit cell-input--manual';gpsInput.placeholder='GPS Elev';gpsInput.inputMode='decimal';if(isNum(r.gpsElev)){gpsInput.value=num3(r.gpsElev)}else{gpsInput.value=''}
gpsInput.addEventListener('blur',()=>{const raw=(gpsInput.value??'').trim();if(!raw){r.gpsElev=null}else{const v=mmToMeters(raw);r.gpsElev=isNum(v)?v:null}
saveGradeSpots();renderGradeSpots()});tdGps.appendChild(gpsInput);let gpsInv=null;const gpsLine=document.createElement('div');gpsLine.className='mini-hint';if(isNum(r.gpsElev)){if(r.gpsShotMode==='inv'){gpsInv=r.gpsElev;gpsLine.innerHTML=`GPS invert = <b>${num3(gpsInv)}</b>`}else{const gpsDrop=getGpsDropForRow(r);if(isNum(gpsDrop)){gpsInv=r.gpsElev-gpsDrop;gpsLine.innerHTML=`GPS ${num3(r.gpsElev)} − ${num3(gpsDrop)} = <b>${num3(gpsInv)}</b>`}else{gpsLine.textContent=`GPS ${num3(r.gpsElev)} → inv —`}}}else{gpsLine.textContent='GPS —'}
tdGps.appendChild(gpsLine);const gpsDeltaLine=document.createElement('div');gpsDeltaLine.className='mini-hint';if(isNum(r.dInv)&&isNum(gpsInv)){gpsDeltaLine.innerHTML=deltaMmText(gpsInv,r.dInv)}else{gpsDeltaLine.textContent='Δ —'}
tdGps.appendChild(gpsDeltaLine);const gpsModeChip=document.createElement('span');gpsModeChip.className='grade-chip grade-chip--active';gpsModeChip.textContent=(r.gpsShotMode==='inv')?'GPS INV':'GPS TOP';gpsModeChip.style.marginTop='2px';gpsModeChip.addEventListener('click',()=>{r.gpsShotMode=(r.gpsShotMode==='inv')?'top':'inv';saveGradeSpots();renderGradeSpots()});tdGps.appendChild(gpsModeChip);tr.appendChild(tdGps);els.gradeSpotsBody.appendChild(tr)})}
